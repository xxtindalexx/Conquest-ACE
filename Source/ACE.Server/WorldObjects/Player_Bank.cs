...
// using Google.Protobuf.WellKnownTypes;  // CONQUEST: Removed ILT dependency
// using MySqlX.XDevAPI;  // CONQUEST: Removed ILT dependency
...
namespace ACE.Server.WorldObjects
{
    partial class Player
    {
        ...
        // CONQUEST: DepositMythicalKeys removed - Mythical Keys not used in Conquest
        ...
        public void DepositConquestCoins(bool suppressChat = false)
        {
            if (ConquestCoins == null)
            {
                ConquestCoins = 0;
            }
            lock (balanceLock)
            {
                long totalDeposited = 0;
                //int i = 0;
                var coinList = this.GetInventoryItemsOfWCID(13370001); // TODO: Update WCID for Conquest Coins
                ...
                if (!suppressChat)
                {
                    if (totalDeposited > 0)
                    {
                        Session.Network.EnqueueSend(new GameMessageSystemChat($"Deposited {totalDeposited:N0} Conquest Coins", ChatMessageType.System));
                    }
                    else
                    {
                        Session.Network.EnqueueSend(new GameMessageSystemChat("No Conquest Coins found to deposit", ChatMessageType.System));
                    }
                }
            }
        ...
        public void DepositSoulFragments(bool suppressChat = false)
        {
            ...
            lock (balanceLock)
            {
                long totalDeposited = 0;
                //int i = 0;
                var fragmentList = this.GetInventoryItemsOfWCID(13370003); // TODO: Update WCID for Soul Fragments
                foreach (var coin in fragmentList)
                {
                    int val = coin.Value ?? 0;
                    if (val > 0)
                    {
                ...
                if (!suppressChat)
                {
                    if (totalDeposited > 0)
                    {
                        Session.Network.EnqueueSend(new GameMessageSystemChat($"Deposited {totalDeposited:N0} Soul Fragments", ChatMessageType.System));
                    }
                    else
                    {
        ...
        // CONQUEST: CreateMythicalKey removed - Mythical Keys not used in Conquest

        // CONQUEST: WithdrawMythicalKeys removed - Mythical Keys not used in Conquest
        ...
        /// <summary>
        /// Creates Conquest Coins in multiple stacks respecting maximum stack size limits.
        /// Uses batched network updates for optimal performance.
        /// </summary>
        ...
        public void WithdrawConquestCoins(long Amount)
        {
            if (Amount <= 0)
            {
                Session.Network.EnqueueSend(new GameMessageSystemChat("Amount must be greater than zero", ChatMessageType.System));
            ...
            // Check if player has enough Conquest Coins (outside lock for early exit)
            if (ConquestCoins < Amount)
            {
                Session.Network.EnqueueSend(new GameMessageSystemChat($"You don't have enough Conquest Coins banked. Need {Amount:N0} coins but only have {ConquestCoins:N0}.", ChatMessageType.System));
                return;
            }
            ...
            if (successfullyCreated > 0)
            {
                lock (balanceLock)
                {
                    ConquestCoins -= successfullyCreated;
                }
                
                // Send notifications outside of lock
                if (successfullyCreated == Amount)
                {
                    Session.Network.EnqueueSend(new GameMessageSystemChat($"Withdrew {successfullyCreated:N0} Conquest Coins", ChatMessageType.System));
                }
                else
                {
                    Session.Network.EnqueueSend(new GameMessageSystemChat($"Withdrew {successfullyCreated:N0} Conquest Coins (partial - insufficient pack space for remaining {Amount - successfullyCreated:N0} Conquest Coins)", ChatMessageType.System));
                }
            }
            else
            {
                Session.Network.EnqueueSend(new GameMessageSystemChat("Failed to create Conquest Coins - check pack space. Withdrawal cancelled.", ChatMessageType.System));
            }
        }

        /// <summary>
        /// Creates weakly Conquest Coins in multiple stacks respecting maximum stack size limits.
        /// Uses batched network updates for optimal performance.
        /// </summary>
        ...
        public void WithdrawSoulFragments(long Amount)
        {
            ...
            if (successfullyCreated > 0)
            {
                lock (balanceLock)
                {
                    SoulFragments -= successfullyCreated;
                }
        ...
        public bool TransferConquestCoins(long Amount, string CharacterDestination)
        {
            ...
            // Check if player has enough Conquest Coins to transfer
            if (ConquestCoins < Amount)
            {
                Session.Network.EnqueueSend(new GameMessageSystemChat($"You don't have enough Conquest Coins banked to transfer. Need {Amount:N0} but only have {ConquestCoins:N0}.", ChatMessageType.System));
                return false;
            }
        ...
        public bool TransferSoulFragments(long Amount, string CharacterDestination)
        {
            ...
            // Check if player has enough weakly Conquest Coins to transfer
            if (SoulFragments < Amount)
            {
                Session.Network.EnqueueSend(new GameMessageSystemChat($"You don't have enough weakly Conquest Coins banked to transfer. Need {Amount:N0} but only have {SoulFragments:N0}.", ChatMessageType.System));
                return false;
            }
            ...
            else
            {
                if (tarplayer is OfflinePlayer)
                {
                    ...
                    // Add to offline receiver
                    if (offlinePlayer.SoulFragments == null)
                    {
                        offlinePlayer.SoulFragments = Amount;
                    }
                    else
                    {
                        offlinePlayer.SoulFragments += Amount;
                    ...
                    // Send confirmation to sender
                    Session.Network.EnqueueSend(new GameMessageSystemChat($"Transferred {Amount:N0} Weakly Conquest Coins to {offlinePlayer.Name} (offline)", ChatMessageType.System));
                }
                else
                {
                    var onlinePlayer = (Player)tarplayer;
                    ...
                    lock (firstLock)
                    {
                        lock (secondLock)
                        {
                            // Perform atomic transfer
                            this.SoulFragments -= Amount;
                            onlinePlayer.SoulFragments = (onlinePlayer.SoulFragments ?? 0) + Amount;
                        }
                    }
                    
                    // Send notification outside of locks
                    onlinePlayer.Session.Network.EnqueueSend(new GameMessageSystemChat($"Received {Amount:N0} Weakly Conquest Coins from {this.Name}", ChatMessageType.System));
                    Session.Network.EnqueueSend(new GameMessageSystemChat($"Transferred {Amount:N0} Weakly Conquest Coins to {onlinePlayer.Name}", ChatMessageType.System));
                    // Persist to database for significant transfers (performance optimization)
                    if (Amount > 10)
                    {
                        onlinePlayer.SavePlayerToDatabase();
                ...
                //Session.Network.EnqueueSend(new GameMessagePrivateUpdatePropertyInt64(this, PropertyInt64.SoulFragments, this.SoulFragments ?? 0));
                // Persist to database for significant transfers (performance optimization)
                if (Amount > 10)
                {
                    this.SavePlayerToDatabase();
                ...
                // Log the transfer
                TransferLogger.LogBankTransfer(this, CharacterDestination, "Soul Fragments", Amount, TransferLogger.TransferTypeBankTransfer);
