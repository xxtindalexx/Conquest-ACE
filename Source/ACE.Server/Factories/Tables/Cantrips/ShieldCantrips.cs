...
{
    // CONQUEST: Shield cantrip table — includes Heart Seeker / Defender for main-hand weapon bonus via wielder aura
    public static class ShieldCantrips
    {
        ...
        private static readonly List<SpellId> spells = new List<SpellId>()
        {
            ...
            // CONQUEST: weapon cantrips — apply to wielder aura, benefit main-hand melee
        ...
        private static void BuildSpells()
        {
            ...
            for (var i = 0; i < spells.Count; i++)
            {
                ...
                if (spellLevels == null)
                {
                    log.Error($"ShieldCantrips - couldn't find {spell}");
                    continue;
                }

                if (spellLevels.Count != NumLevels)
                {
                    log.Error($"ShieldCantrips - expected {NumLevels} levels for {spell}, found {spellLevels.Count}");
                    continue;
                }
