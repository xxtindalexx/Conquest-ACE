    ...
    public class Fellowship
    {
        ...
        public void AddFellowshipMember(Player inviter, Player newMember)
        {
            ...
            if (IsLocked)
            {
                ...
                else
                {
                    ...
                    var timeLimit = Time.GetDateTimeFromTimestamp(timeDeparted).AddSeconds(360); // 6 minutes for locked fellowships
        ...
        public void UpdateLock(bool isLocked, string lockName)
        {
            ...
            if (isLocked)
            {
                ...
                SendBroadcastAndUpdate("Your fellowship is now locked.  You may not recruit new members.  If you leave the fellowship, you have 6 minutes to be recruited back into the fellowship.");
            }
            else
            {
        ...
        public void ProcessDropList(Dictionary<uint, WeakReference<Player>> fellowshipMembers, HashSet<uint> fellowGuids)
        {
            foreach (var fellowGuid in fellowGuids)
            {
                ...
                // CONQUEST: Track departure time so crashed players can rejoin
                var timestamp = (int)Time.GetUnixTime();
