    ...
    public class ArenaLocation
    {
        ...
        public void Tick()
        {
            ...
            if (HasActiveEvent)
            {
                ...
                if (PropertyManager.GetBool("disable_arenas"))
                {
        ...
        public void EndEventWithWinner(Guid winningTeamGuid)
        {
            ...
            {
                var player = PlayerManager.GetOnlinePlayer(arenaPlayer.CharacterId);
                if (player != null && player.Age <= PropertyManager.GetLong("arenas_reward_min_age")) //days in seconds
                {
                    underageCount++;
                }
            }
                    ...
                    if (shouldReward)
                    {
                        switch (ActiveEvent.EventType)
                        {
                            ...
                            case "2v2":
                                ...
                                {
                                    soulFragment.SetStackSize(1);
                                    var soulFragmentCreateResult = player.TryCreateInInventoryWithNetworking(soulFragment);
                                    if (soulFragmentCreateResult)
                                    {
                                        player.Session.Network.EnqueueSend(new GameMessageCreateObject(soulFragment));
                                        var msg = new GameMessageSystemChat($"You have received a Soul Fragment", ChatMessageType.Broadcast);
                                        player.Session.Network.EnqueueSend(msg);
                                    }
                                }

                                //Give 1 Aged Legendary Key
                                var legendaryKey = WorldObjectFactory.CreateNewWorldObject(48746); //Aged Legendary Key
                                if (legendaryKey != null)
                                {
                                    legendaryKey.SetStackSize(1);
                                    var legendaryKeyCreateResult = player.TryCreateInInventoryWithNetworking(legendaryKey);
                                    if (legendaryKeyCreateResult)
                                    {
                                        player.Session.Network.EnqueueSend(new GameMessageCreateObject(legendaryKey));
                                        var msg = new GameMessageSystemChat($"You have received an Aged Legendary Key", ChatMessageType.Broadcast);
                                        player.Session.Network.EnqueueSend(msg);
                                    }
                                }
                                break;
                            ...
                            case "tugak":
                                ...
                                if (ffaWinnerSoulFragment != null)
                                {
                                    ffaWinnerSoulFragment.SetStackSize(1);
                                    var ffaWinnerSoulFragmentCreateResult = player.TryCreateInInventoryWithNetworking(ffaWinnerSoulFragment);
                                    if (ffaWinnerSoulFragmentCreateResult)
                                    {
                                        player.Session.Network.EnqueueSend(new GameMessageCreateObject(ffaWinnerSoulFragment));
                                        var msg = new GameMessageSystemChat($"You have received a Soul Fragment", ChatMessageType.Broadcast);
                                        player.Session.Network.EnqueueSend(msg);
                                    }
                                }
                                ...
                                //Give 1 Aged Legendary Key
                                var ffaWinnerLegendaryKey = WorldObjectFactory.CreateNewWorldObject(48746); //Aged Legendary Key
                                if (ffaWinnerLegendaryKey != null)
                                {
                                    ffaWinnerLegendaryKey.SetStackSize(1);
                                    var ffaWinnerLegendaryKeyCreateResult = player.TryCreateInInventoryWithNetworking(ffaWinnerLegendaryKey);
                                    if (ffaWinnerLegendaryKeyCreateResult)
                                    {
                                        player.Session.Network.EnqueueSend(new GameMessageCreateObject(ffaWinnerLegendaryKey));
                                        var msg = new GameMessageSystemChat($"You have received an Aged Legendary Key", ChatMessageType.Broadcast);
                                        player.Session.Network.EnqueueSend(msg);
                                    }
                                }
                            ...
                            case "group":
                                ...
                                if (groupWinnerSoulFragment != null)
                                {
                                    groupWinnerSoulFragment.SetStackSize(1);
                                    var groupWinnerSoulFragmentCreateResult = player.TryCreateInInventoryWithNetworking(groupWinnerSoulFragment);
                                    if (groupWinnerSoulFragmentCreateResult)
                                    {
                                        player.Session.Network.EnqueueSend(new GameMessageCreateObject(groupWinnerSoulFragment));
                                        var msg = new GameMessageSystemChat($"You have received a Soul Fragment", ChatMessageType.Broadcast);
                                        player.Session.Network.EnqueueSend(msg);
                                    }
                                }
                                ...
                                //Give 1 Aged Legendary Key
                                var groupWinnerLegendaryKey = WorldObjectFactory.CreateNewWorldObject(48746); //Aged Legendary Key
                                if (groupWinnerLegendaryKey != null)
                                {
                                    groupWinnerLegendaryKey.SetStackSize(1);
                                    var groupWinnerLegendaryKeyCreateResult = player.TryCreateInInventoryWithNetworking(groupWinnerLegendaryKey);
                                    if (groupWinnerLegendaryKeyCreateResult)
                                    {
                                        player.Session.Network.EnqueueSend(new GameMessageCreateObject(groupWinnerLegendaryKey));
                                        var msg = new GameMessageSystemChat($"You have received an Aged Legendary Key", ChatMessageType.Broadcast);
                                        player.Session.Network.EnqueueSend(msg);
                                    }
                                }
        ...
        public void ClearPlayersFromArena()
        {
            ...
            try
            {
                var arenaLandblock = LandblockManager.GetLandblock(new LandblockId(LandblockId << 16), false, 2, false);
            ...
            get
            {
                if (_arenaLocationStartingPositions == null)
                {
                    ...
                    _arenaLocationStartingPositions.Add(
                        ...
                        new List<Position>()
                        {
                            new Position(0x00670103, 0f, -30f, 0.004999995f, 0f, 0f, -0.699713f, 0.714424f, false, 2), //West
                            //0x00670103 [0 -30 0.004999995] 0.714424 0 0 -0.699713
                            new Position(0x00670127, 62.829544f, -28.933987f, 0.005000f, 0f, 0f, 0.654449f, 0.756106f, false, 2), //East
                            //0x00670127 [62.829544 -28.933987 0.005000] 0.756106 0.000000 0.000000 0.654449
                            new Position(0x00670117, 29.432631f, -49.517181f, 0.005000f, 0f, 0f, -0.036533f, 0.999332f, false, 2), //South
                            //0x00670117 [29.432631 -49.517181 0.005000] 0.999332 0.000000 0.000000 -0.036533
                            new Position(0x00670112, 29.911026f, -0.265875f, 0.005000f, 0f, 0f, 0.999908f, 0.013594f, false, 2), //North
                            //0x00670112 [29.911026 -0.265875 0.005000] 0.013594 0.000000 0.000000 0.999908
                            new Position(0x00670105, 2.247503f, -48.398197f, 0.005000f, 0f, 0f, -0.440161f, 0.897919f, false, 2), //SW
                            //0x00670105 [2.247503 -48.398197 0.005000] 0.897919 0.000000 0.000000 -0.440161
                            new Position(0x00670124, 58.684017f, -2.170248f, 0.005000f, 0f, 0f, 0.902263f, 0.431186f, false, 2), //NE
                            //0x00670124 [58.684017 -2.170248 0.005000] 0.431186 0.000000 0.000000 0.902263
                            new Position(0x00670129, 58.118492f, -47.702240f, 0.005000f, 0f, 0f, 0.426291f, 0.904586f, false, 2), //SE
                            //0x00670129 [58.118492 -47.702240 0.005000] 0.904586 0.000000 0.000000 0.426291
                            new Position(0x00670100, 1.902291f, -2.042494f, 0.005000f, 0f, 0f, 0.914624f, -0.404305f, false, 2), //NW
                            //0x00670100 [1.902291 -2.042494 0.005000] -0.404305 0.000000 0.000000 0.914624
                            new Position(0x00670109, 14.541056f, -26.421442f, 0.005000f, 0f, 0f, 0.701743f, -0.712430f, false, 2), //MidW
                            //0x00670109 [14.541056 -26.421442 0.005000] -0.712430 0.000000 0.000000 0.701743
                            new Position(0x0067011A, 37.242702f, -24.179514f, 0.005000f, 0f, 0f, 0.728694f, 0.684840f, false, 2), //MidE
                    ...
                    _arenaLocationStartingPositions.Add(
                        ...
                        new List<Position>()
                        {
                            new Position(0x007F0101, 236.75943f, -22.896727f, -59.995f, 0.000000f, 0.000000f, -0.6899546f, 0.72385263f, false, 2), //West
                            //0x007F0101 [236.75943 -22.896727 -59.995] 0.72385263 0 0 -0.6899546
                            new Position(0x007F0107, 262.50168f, -16.419994f, -59.995f, 0.000000f, 0.000000f, -0.8109761f, -0.58507925f, false, 2),//East
                            //0x007F0107 [262.50168 -16.419994 -59.995] -0.58507925 0 0 -0.8109761
                            new Position(0x007F0103, 250.935226f, -5.749045f, -59.994999f, 0.000000f, 0.000000f, -0.999384f, -0.035106f, false, 2),//North
                            //0x007F0103 [250.935226 -5.749045 -59.994999] 0.035106 0.000000 0.000000 -0.999384
                            new Position(0x007F0105, 252.860062f, -33.350712f, -59.994999f, 0.000000f, 0.000000f, -0.043396f, -0.999058f, false, 2),//South
                    ...
                    _arenaLocationStartingPositions.Add(
                        ...
                        new List<Position>()
                        {
                            new Position(0x02E30100, 54.954102f, -53.559502f, 0.005000f, 0.000000f, 0.000000f, -0.706410f, 0.707802f, false, 2),
                            //0x02E30100[54.954102 -53.559502 0.005000] 0.707802 0.000000 0.000000 -0.706410
                            new Position(0x02E30185, 87.640144f, -54.995811f, 12.004999f, 0.000000f, 0.000000f, -0.701326f, -0.712840f, false, 2),
                            //0x02E30185[87.640144 -54.995811 12.004999] -0.712840 0.000000 0.000000 -0.701326
                            new Position(0x02E3014A, 54.859180f, -20.170738f, 12.004999f, 0.000000f, 0.000000f, -0.999738f, -0.022893f, false, 2),
                            //0x02E3014A[54.859180 -20.170738 12.004999] -0.022893 0.000000 0.000000 -0.999738
                            new Position(0x02E30131, 21.610455f, -54.913197f, 12.004999f, 0.000000f, 0.000000f, -0.714008f, 0.700137f, false, 2),
                            //0x02E30131[21.610455 -54.913197 12.004999] 0.700137 0.000000 0.000000 -0.714008
                            new Position(0x02E3015D, 55.055714f, -88.516785f, 12.004999f, 0.000000f, 0.000000f, 0.008961f, 0.999960f, false, 2),
                            //0x02E3015D[55.055714 -88.516785 12.004999] 0.999960 0.000000 0.000000 0.008961
                            new Position(0x02E30107, 37.609787f, -55.014618f, 6.005000f, 0.000000f, 0.000000f, -0.714715f, 0.699416f, false, 2),
                            //0x02E30107[37.609787 -55.014618 6.005000] 0.699416 0.000000 0.000000 -0.714715
                            new Position(0x02E30112, 55.120712f, -72.074440f, 6.005000f, 0.000000f, 0.000000f, 0.013379f, 0.999910f, false, 2),
                            //0x02E30112[55.120712 -72.074440 6.005000] 0.999910 0.000000 0.000000 0.013379
                            new Position(0x02E30119, 71.587006f, -55.205040f, 6.005000f, 0.000000f, 0.000000f, 0.688900f, 0.724856f, false, 2),
                            //0x02E30119[71.587006 -55.205040 6.005000] 0.724856 0.000000 0.000000 0.688900
                            new Position(0x02E3010F, 55.080246f, -37.959023f, 6.005000f, 0.000000f, 0.000000f, 0.999951f, 0.009948f, false, 2),
                            //0x02E3010F[55.080246 -37.959023 6.005000] 0.009948 0.000000 0.000000 0.999951
                            new Position(0x02E3015A, 59.596912f, -55.734844f, 12.054999f, 0.000000f, 0.000000f, -0.717913f, -0.696133f, false, 2),
                    ...
                    //    new List<Position>()
                    //    {
                    //        new Position(0xECEC012D, 84.326401f, 37.399399f, 0.205000f, 0.000000f, 0.000000f, -0.010572f, -0.999944f, false, 2),
                    //        //0xECEC012D[84.326401 37.399399 0.205000] -0.999944 0.000000 0.000000 -0.010572
                    //        new Position(0xECEC011F, 131.125320f, 36.631672f, 0.205000f, 0.000000f, 0.000000f, -0.333597f, -0.942716f, false, 2),
                    //        //0xECEC011F[131.125320 36.631672 0.205000] -0.942716 0.000000 0.000000 -0.333597
                    //        new Position(0xECEC012C, 132.890320f, 84.706825f, 0.205000f, 0.000000f, 0.000000f, -0.717156f, -0.696913f, false, 2),
                    //        //0xECEC012C[132.890320 84.706825 0.205000] -0.696913 0.000000 0.000000 -0.717156
                    //        new Position(0xECEC0122, 131.195908f, 130.575638f, 0.205000f, 0.000000f, 0.000000f, -0.916779f, -0.399395f, false, 2),
                    //        //0xECEC0122[131.195908 130.575638 0.205000] -0.399395 0.000000 0.000000 -0.916779
                    //        new Position(0xECEC012B, 83.108978f, 131.664230f, 0.205000f, 0.000000f, 0.000000f, -0.999970f, 0.007704f, false, 2),
                    //        //0xECEC012B[83.108978 131.664230 0.205000] 0.007704 0.000000 0.000000 -0.999970
                    //        new Position(0xECEC0121, 37.388470f, 130.349777f, 0.205000f, 0.000000f, 0.000000f, -0.926284f, 0.376825f, false, 2),
                    //        //0xECEC0121[37.388470 130.349777 0.205000] 0.376825 0.000000 0.000000 -0.926284
                    //        new Position(0xECEC012E, 36.323086f, 83.237755f, 0.205000f, 0.000000f, 0.000000f, -0.679018f, 0.734121f, false, 2),
                    //        //0xECEC012E[36.323086 83.237755 0.205000] 0.734121 0.000000 0.000000 -0.679018
                    //        new Position(0xECEC0120, 37.112888f, 37.668873f, 0.205000f, 0.000000f, 0.000000f, -0.401558f, 0.915833f, false, 2),
                    //        //0xECEC0120[37.112888 37.668873 0.205000] 0.915833 0.000000 0.000000 -0.401558
                    //        new Position(0xECEC0105, 72.137177f, 84.142395f, 0.205000f, 0.000000f, 0.000000f, -0.702030f, 0.712147f, false, 2),
                    //        //0xECEC0105[72.137177 84.142395 0.205000] 0.712147 0.000000 0.000000 -0.702030
                    //        new Position(0xECEC0110, 97.596474f, 83.986618f, 0.205000f, 0.000000f, 0.000000f, 0.715533f, 0.698579f, false, 2),
                    ...
                    _arenaLocationStartingPositions.Add(
                        ...
                        new List<Position>()
                        {
                            new Position(0x01AD0116, 29.940001f, -32.599998f, 0.005000f, 0.000000f, 0.000000f, -0.008727f, 0.999962f, false, 2),
                            //0x01AD0116[29.940001 -32.599998 0.005000] 0.999962 0.000000 0.000000 -0.008727
                            new Position(0x01AD011A, 40.046265f, -1.053921f, 0.005000f, 0.000000f, 0.000000f, -0.902557f, -0.430570f, false, 2),
                            //0x01AD011A[40.046265 -1.053921 0.005000] -0.430570 0.000000 0.000000 -0.902557
                            new Position(0x01AD0100, 0.060270f, 2.052596f, 0.005000f, 0.000000f, 0.000000f, -0.929969f, 0.367637f, false, 2),
                            //0x01AD0100[0.060270 2.052596 0.005000] 0.367637 0.000000 0.000000 -0.929969
                            new Position(0x01AD0142, 39.877190f, -9.612027f, 6.005000f, 0.000000f, 0.000000f, -0.999993f, 0.003741f, false, 2),
                            //0x01AD0142[39.877190 -9.612027 6.005000] 0.003741 0.000000 0.000000 -0.999993
                            new Position(0x01AD014B, 61.338745f, 1.616516f, 6.005000f, 0.000000f, 0.000000f, -0.923844f, -0.382769f, false, 2),
                            //0x01AD014B[61.338745 1.616516 6.005000] -0.382769 0.000000 0.000000 -0.923844
                            new Position(0x01AD014E, 59.279133f, -31.289797f, 6.005000f, 0.000000f, 0.000000f, -0.310898f, -0.950443f, false, 2),
                            //0x01AD014E[59.279133 -31.289797 6.005000] -0.950443 0.000000 0.000000 -0.310898
                            new Position(0x01AD0144, 40.064552f, -25.380503f, 6.005000f, 0.000000f, 0.000000f, -0.017525f, -0.999846f, false, 2),
                            //0x01AD0144[40.064552 -25.380503 6.005000] -0.999846 0.000000 0.000000 -0.017525
                            new Position(0x01AD0135, 18.353422f, -20.375107f, 6.005000f, 0.000000f, 0.000000f, 0.872478f, -0.488653f, false, 2),
                            //0x01AD0135[18.353422 -20.375107 6.005000] -0.488653 0.000000 0.000000 0.872478
                            new Position(0x01AD016E, 40.068432f, 0.782179f, 18.004999f, 0.000000f, 0.000000f, -0.919731f, -0.392550f, false, 2),
                            //0x01AD016E[40.068432 0.782179 18.004999] -0.392550 0.000000 0.000000 -0.919731
                            new Position(0x01AD0155, 30.068464f, -20.947460f, 12.004999f, 0.000000f, 0.000000f, -0.999852f, 0.017178f, false, 2),
                    ...
                    _arenaLocationStartingPositions.Add(
                        ...
                        new List<Position>()
                        {
                            new Position(0x014501A3, 96.569344f, -50.926197f, 6.005000f, 0.000000f, 0.000000f, -0.189269f, -0.981925f, false, 2),
                            //0x014501A3[96.569344 -50.926197 6.005000] -0.981925 0.000000 0.000000 -0.189269
                            new Position(0x014501A1, 97.130882f, -27.609722f, 6.005000f, 0.000000f, 0.000000f, -0.953241f, -0.302211f, false, 2),
                            //0x014501A1[97.130882 -27.609722 6.005000] -0.302211 0.000000 0.000000 -0.953241
                            new Position(0x0145010E, 100.117851f, -50.360348f, -5.995000f, 0.000000f, 0.000000f, 0.003506f, 0.999994f, false, 2),
                            //0x0145010E[100.117851 -50.360348 -5.995000] 0.999994 0.000000 0.000000 0.003506
                            new Position(0x0145010B, 99.959366f, -29.854738f, -5.995000f, 0.000000f, 0.000000f, 0.999639f, 0.026883f, false, 2),
                            //0x0145010B[99.959366 -29.854738 -5.995000] 0.026883 0.000000 0.000000 0.999639
                            new Position(0x01450149, 71.072929f, -26.698454f, 0.005000f, 0.000000f, 0.000000f, -0.996001f, 0.089337f, false, 2),
                            //0x01450149[71.072929 -26.698454 0.005000] 0.089337 0.000000 0.000000 -0.996001
                            new Position(0x0145014B, 72.077400f, -52.935970f, 0.005000f, 0.000000f, 0.000000f, -0.170279f, 0.985396f, false, 2),
                            //0x0145014B[72.077400 -52.935970 0.005000] 0.985396 0.000000 0.000000 -0.170279
                            new Position(0x01450117, 29.986937f, -13.254610f, 0.005000f, 0.000000f, 0.000000f, 0.999936f, 0.011354f, false, 2),
                            //0x01450117[29.986937 -13.254610 0.005000] 0.011354 0.000000 0.000000 0.999936
                            new Position(0x01450185, 29.381468f, -40.134899f, 6.005000f, 0.000000f, 0.000000f, 0.686305f, -0.727314f, false, 2),
                            //0x01450185[29.381468 -40.134899 6.005000] -0.727314 0.000000 0.000000 0.686305
                            new Position(0x01450121, 29.054979f, -57.933998f, 0.005000f, 0.000000f, 0.000000f, 0.015373f, 0.999882f, false, 2),
                            //0x01450121[29.054979 -57.933998 0.005000] 0.999882 0.000000 0.000000 0.015373
                            new Position(0x01450114, 5.084766f, -76.363258f, 0.005000f, 0.000000f, 0.000000f, -0.394789f, 0.918772f, false, 2),
                    ...
                    //new List<Position>()
                    //{
                    //        new Position(0x00AF015D, 201.930344f, -47.542015f, 0.110000f, 0.000000f, 0.000000f, 0.000000f, 1.000000f, false, 2),
                    //        //0x00AF015D [201.930344 -47.542015 0.110000] 1.000000 0.000000 0.000000 0.000000
                    //        new Position(0x00AF015B, 201.882141f, -32.338894f, 0.110000f, 0.000000f, 0.000000f, 0.999997f, -0.002561f, false, 2),
                    //        //0x00AF015B [201.882141 -32.338894 0.110000] -0.002561 0.000000 0.000000 0.999997
                    //        new Position(0x00AF015D, 198.203186f, -47.391590f, 0.110000f, 0.000000f, 0.000000f, 0.010101f, 0.999949f, false, 2),
                    //        //0x00AF015D [198.203186 -47.391590 0.110000] 0.999949 0.000000 0.000000 0.010101
                    //        new Position(0x00AF015B, 198.134445f, -32.373367f, 0.110000f, 0.000000f, 0.000000f, -0.999939f, -0.011021f, false, 2),
                    ...
                    new List<Position>()
                    {
                            new Position(0x596A010C, 35.238209f, -18.218103f, 0.005000f, 0.000000f, 0.000000f, -0.707107f, -0.707107f, false, 2),
                            //0x596A010C [35.238209 -18.218103 0.005000] -0.707107 0.000000 0.000000 -0.707107
                            new Position(0x596A0102, 5.035136f, -18.326546f, 0.005000f, 0.000000f, 0.000000f, -0.695197f, 0.718819f, false, 2),
                            //0x596A0102 [5.035136 -18.326546 0.005000] 0.718819 0.000000 0.000000 -0.695197
                            new Position(0x596A010A, 34.787258f, -21.811909f, 0.005000f, 0.000000f, 0.000000f, 0.713734f, 0.700416f, false, 2),
                            //0x596A010A [34.787258 -21.811909 0.005000] 0.700416 0.000000 0.000000 0.713734
                            new Position(0x596A0102, 5.146578f, -21.661127f, 0.005000f, 0.000000f, 0.000000f, -0.695197f, 0.718819f, false, 2),
